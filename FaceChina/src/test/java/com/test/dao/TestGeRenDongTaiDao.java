...
public class TestGeRenDongTaiDao extends TestBase{
	...
	/*	//测试获取呈现到个人动态的文字内容、更新时间和图片数据
		List<GeRenShuoVO> shuolist = dao.findGeRenShuo(2, 0, 2);
		for (GeRenShuoVO geRenShuoVO : shuolist) {
			System.out.println(geRenShuoVO);
		}*/
		
		//测试获取呈现到个人动态的每个说说当中的评论者id、评论文字内容、评论更新时间
		List<GeRenPingLunVO> pinglist = dao.findGeRenPingLun(6);
		for (GeRenPingLunVO geRenPingLunVO : pinglist) {
			System.out.println(geRenPingLunVO);
		}
		/*
		//测试个人动态左上角个人所有的说说总篇数
